...
namespace GreedyVox.NetCode.Ai
{
    ...
    public class NetCodeAnimatorAiMonitor : AnimatorMonitor
    {
        /// <summary>
        ...
        private void InitializeItemParameters(ref FastBufferReader reader)
        {
            ...
            SetItemStateIndexParameter(idx, state, true);
            SetItemSubstateIndexParameter(idx, index, true);
            SnapAnimator();
        }
        /// <summary>
            ...
            if (HasItemParameters)
            {
                ...
                for (int i = 0; i < ParameterSlotCount; i++)
                {
                    if ((slot & (i + 1)) != 0)
                    {
                        ...
                        SetItemStateIndexParameter(i, state, true);
                        ByteUnpacker.ReadValuePacked(reader, out index);
                        SetItemSubstateIndexParameter(i, index, true);
                    }
                }
            }
        }
        ...
        /// <param name="value">The new value.</param>
        /// <returns>True if the parameter was changed.</returns>
        public override bool SetItemStateIndexParameter(int slotID, int value, bool force)
        {
            // The animator may not be enabled. Return silently.
            if (m_Animator != null && !m_Animator.isActiveAndEnabled)
            {
            ...
            if (base.SetItemStateIndexParameter(slotID, value, force))
            {
                m_ItemDirtySlot |= (byte)(slotID + 1);
                return true;
            }
        ...
        /// <param name="value">The new value.</param>
        /// <returns>True if the parameter was changed.</returns>
        public override bool SetItemSubstateIndexParameter(int slotID, int value, bool force)
        {
            // The animator may not be enabled. Return silently.
            if (m_Animator != null && !m_Animator.isActiveAndEnabled)
            {
            ...
            if (base.SetItemSubstateIndexParameter(slotID, value, force))
            {
                m_ItemDirtySlot |= (byte)(slotID + 1);
                return true;
            }
